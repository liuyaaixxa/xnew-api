...
func migrateDBFast() error {
	...
	migrations := []struct {
		...
		{&AffiliatePromotion{}, "AffiliatePromotion"},
	}
...
` + "`max_device_tokens`" + ` integer DEFAULT 0,
` + "`max_user_channels`" + ` integer DEFAULT 0,
` + "`max_shared_models`" + ` integer DEFAULT 0,
...
PRIMARY KEY (` + "`id`" + `)
	...
	required := []sqliteColumnDef{
		...
		{Name: "max_device_tokens", DDL: "`max_device_tokens` integer DEFAULT 0"},
		{Name: "max_user_channels", DDL: "`max_user_channels` integer DEFAULT 0"},
		{Name: "max_shared_models", DDL: "`max_shared_models` integer DEFAULT 0"},
